...
class GaussExpFalloffKernel(torch.nn.Module):
    ...
    def forward(self, vortex_feature, points):
        ...
        # exp1 = torch.exp(-2.0 * self.dt**2 * sq_distance / (src_sigma**2))
        # exponential_part = torch.exp(-(src_strength / src_sigma)**2 * exp1) / torch.sqrt(sq_distance + src_strength**2 * exp1)
