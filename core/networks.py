...
class SimpleNN(torch.nn.Module):
    ...
    def __init__(self, depth=1, hidden_units=100, in_features=4, out_features=4):
...
class MultiParticleVortexNetwork(torch.nn.Module):
    ...
    def __init__(self, depth=1, hidden_units=100, order=1):
    ...
    def forward(self, inp):
        ...
        for i in range(nparticles):
            ...
            if self.order == 0:
                ...
                dv_dy = torch.autograd.grad(torch.unbind(vel_y, dim=-1), py, create_graph=True, retain_graph=True,
                                            allow_unused=True)[0]
                dv_dx = torch.autograd.grad(torch.unbind(vel_y, dim=-1), px, create_graph=True, retain_graph=True,
                                            allow_unused=True)[0]
                du_dy = torch.autograd.grad(torch.unbind(vel_x, dim=-1), py, create_graph=True, retain_graph=True,
                                            allow_unused=True)[0]
                du_dx = torch.autograd.grad(torch.unbind(vel_x, dim=-1), px, create_graph=True, retain_graph=True,
                                            allow_unused=True)[0]
                ...
                d2u_dx2 = torch.autograd.grad(torch.unbind(du_dx, dim=-1), px, create_graph=True, retain_graph=True,
                                              allow_unused=True)[0]
                d2u_dy2 = torch.autograd.grad(torch.unbind(du_dy, dim=-1), py, create_graph=True, retain_graph=True,
                                              allow_unused=True)[0]
                d2u_dydx = torch.autograd.grad(torch.unbind(du_dx, dim=-1), py, create_graph=True, retain_graph=True,
                                               allow_unused=True)[0]
                d2v_dy2 = torch.autograd.grad(torch.unbind(dv_dy, dim=-1), py, create_graph=True, retain_graph=True,
                                              allow_unused=True)[0]
                d2v_dx2 = torch.autograd.grad(torch.unbind(dv_dx, dim=-1), px, create_graph=True, retain_graph=True,
                                              allow_unused=True)[0]
                d2v_dxdy = torch.autograd.grad(torch.unbind(dv_dy, dim=-1), px, create_graph=True, retain_graph=True,
                                               allow_unused=True)[0]
...
class InteractionNetwork(torch.nn.Module):
    ...
    def __init__(self, depth=1, hidden_units=100):
...
class MultiStepBoundaryConditionNetwork(torch.nn.Module):
    ...
    def __init__(self, depth=1, hidden_units=100, num_steps=1, order=1):

        super(MultiStepBoundaryConditionNetwork, self).__init__()

        self.single_step_net = BoundaryConditionNetwork(depth=depth, hidden_units=hidden_units, order=order)
...
class MultiStepMultiVortexNetwork(torch.nn.Module):
    ...
    def __init__(self, depth=1, hidden_units=100, num_steps=1, order=1):
        ...
        self.single_step_net = MultiParticleVortexNetwork(depth=depth, hidden_units=hidden_units,  order=order)
        module_list = [self.single_step_net]
...
class MultiStepViscousVortexNetwork(torch.nn.Module):
    ...
    def __init__(self, depth=1, hidden_units=100, num_steps=1, order=1):
        ...
        self.single_step_net = ViscousVortexNetwork(depth=depth, hidden_units=hidden_units, order=order)
        module_list = [self.single_step_net]
...
class MultiStepInteractionNetwork(torch.nn.Module):
    ...
    def __init__(self, depth=1, hidden_units=100, num_steps=1):
...
class MultiStepLoss(torch.nn.Module):
    ...
    def __init__(self, resolution=(128, 128), num_steps=1, batch_size=1):
