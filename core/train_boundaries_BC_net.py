...
parser.add_argument('--ex', type=str, default='p10_b_sb_T0_exp_red_weight_1.0_depth_5_100_batch_16_lr_1e-2_l2_1e-5_r100_4000_2', help='name of the experiment')
...
ckpt_save_dir_BC = os.path.join(logs_dir_BC, 'ckpt')
train_summaries_dir = os.path.join(logs_dir_BC, 'train_summary')
val_summaries_dir = os.path.join(logs_dir_BC, 'val_summary')

for dir in [ckpt_save_dir_BC, train_summaries_dir, val_summaries_dir]:
    if not os.path.isdir(dir):
        os.makedirs(dir)

with open(os.path.join(logs_dir_BC, 'train_config'), 'w') as configfile:
    json.dump(vars(opt), configfile, indent=2)

checkpoints_files = os.listdir(os.path.join(ckpt_dir_VN))
epoch_id = np.argmax(np.array([int(i.split('_')[1]) for i in checkpoints_files]))
ckpt_file = os.path.join(ckpt_dir_VN, checkpoints_files[epoch_id])
...
for epoch in range(start_epoch, opt.epochs):
    ...
    for step in range(steps_per_epoch):
        ...
        y_ph = torch.rand(BATCH_SIZE * NPOINTS_IN, dtype=torch.float32, device='cuda:0').view(BATCH_SIZE, -1) * RESOLUTION[0]
        x_ph = torch.rand(BATCH_SIZE * NPOINTS_IN, dtype=torch.float32, device='cuda:0').view(BATCH_SIZE, -1) * RESOLUTION[1]

        y_b1 = torch.rand(BATCH_SIZE * NPOINTS_B, dtype=torch.float32, device='cuda:0').view(BATCH_SIZE, -1) * RESOLUTION[0]
        x_b1 = torch.tensor(np.concatenate([np.array([0] * (NPOINTS_B // 2)), np.array([RESOLUTION[1]] * (NPOINTS_B // 2))]),
                            dtype=torch.float32, device='cuda:0').view(1, -1).repeat(BATCH_SIZE, 1)

        x_b2 = torch.rand(BATCH_SIZE * NPOINTS_B, dtype=torch.float32, device='cuda:0').view(BATCH_SIZE, -1) * RESOLUTION[1]
        y_b2 = torch.tensor(np.concatenate([np.array([0] * (NPOINTS_B // 2)), np.array([RESOLUTION[0]] * (NPOINTS_B // 2))]),
                            dtype=torch.float32, device='cuda:0').view(1, -1).repeat(BATCH_SIZE, 1)
        ...
        div_ph_loss = torch.sum((div_du_dx_all + div_dv_dy_all) ** 2) / (BATCH_SIZE * div_du_dx_all.shape[-1] * NPOINTS_IN)
        ...
        bc_loss = torch.sum((total_vel_b1_x ** 2 + total_vel_b2_y ** 2)) / (BATCH_SIZE * total_vel_b1_x.shape[-1] * 2 * NPOINTS_B)
        ...
        print('Epoch: {}, Step: {}/{}, Div Loss: {:.4f}, BC Loss: {:.4f}, Loss: {:.4f}'.
              format(epoch, step, steps_per_epoch, div_ph_loss.item(), bc_loss.item(), total_bc_net_loss.item()))
    ...
    if val_loss.item() < val_best:
        ...
        ckpt_path = os.path.join(ckpt_save_dir_BC, ckpt_filename)
