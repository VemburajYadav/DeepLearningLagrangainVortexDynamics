...
# loc = [100.0, 100.0]
location_1 = np.array([80.0, 100.0])
location_2 = np.array([120.0, 100.0])
...
# sigma = np.array([10.0]).reshape((1, 1, 1))
...
fluid = world.add(Fluid(domain=domain, velocity=velocity_0),
                  physics=[IncompressibleFlow(),
                           lambda fluid_1, dt: fluid_1.copied_with(velocity=diffuse(fluid_1.velocity, 1.0 * dt))])
...
plt.show()
fig.savefig(os.path.join(save_dir, 'velocity_N2_fit_.pdf'), format='pdf')
