...
parser.add_argument('--offset', type=list, default=[50, 50], help='neglect regions near boundaries of the '
                                                                  'domain (as list: [24, 24])')
parser.add_argument('--obstacle_size_range', type=list, default=[10, 50], help='neglect regions near boundaries of the '
                                                                  'domain (as list: [24, 24])')
...
parser.add_argument('--save_dir', type=str, default='/home/vemburaj/'
                                                    'data/p2_r_dataset_200x200_4000',
...
OBSTACLE_SIZE = opt.obstacle_size_range
...
obstacle_start_xcoords = np.random.randint(OFFSET[1], RESOLUTION[1] - OFFSET[1], NSAMPLES)
obstacle_start_ycoords = np.random.randint(OFFSET[0], RESOLUTION[0] - OFFSET[0], NSAMPLES)

obstacle_heights = np.random.randint(OBSTACLE_SIZE[0], OBSTACLE_SIZE[1], NSAMPLES)
obstacle_widths = np.random.randint(OBSTACLE_SIZE[0], OBSTACLE_SIZE[1], NSAMPLES)

np.random.shuffle(obstacle_heights)
np.random.shuffle(obstacle_widths)
...
#
    ...
    for i in range(NSAMPLES):
        ...
        dist_from_b1 = obstacle_start_xcoords[i] - OFFSET[1]
        dist_from_b2 = RESOLUTION[1] - OFFSET[1] - (obstacle_start_xcoords[i] + obstacle_widths[i])
        ratio = int(dist_from_b1 / (RESOLUTION[1] - 2 * OFFSET[1]))
        NPARTICLES_1 = int(ratio * NPARTICLES)
        NPARTICLES_2 = NPARTICLES - NPARTICLES_1

        xcoords[i, 0:NPARTICLES_1] = np.random.random_sample(size=(NPARTICLES_1)) * dist_from_b1 + OFFSET[1]
        xcoords[i, NPARTICLES_1: ] = np.random.random_sample(size=(NPARTICLES_2)) * dist_from_b2 \
                                     + obstacle_start_xcoords[i] + obstacle_widths[i]
...
train_obs_ycoords_1, train_obs_xcoords_1 = \
    obstacle_start_ycoords[0 : N_TRAIN_SAMPLES], obstacle_start_xcoords[0 : N_TRAIN_SAMPLES]
train_obs_ycoords_2, train_obs_xcoords_2 = \
    obstacle_start_ycoords[0 : N_TRAIN_SAMPLES] + obstacle_heights[0: N_TRAIN_SAMPLES],\
    obstacle_start_xcoords[0 : N_TRAIN_SAMPLES] + obstacle_widths[0: N_TRAIN_SAMPLES]
...
val_obs_ycoords_1, val_obs_xcoords_1 = \
    obstacle_start_ycoords[N_TRAIN_SAMPLES: (N_TRAIN_SAMPLES + N_VAL_SAMPLES)], \
    obstacle_start_xcoords[N_TRAIN_SAMPLES: (N_TRAIN_SAMPLES + N_VAL_SAMPLES)]
val_obs_ycoords_2, val_obs_xcoords_2 = \
    obstacle_start_ycoords[N_TRAIN_SAMPLES: (N_TRAIN_SAMPLES + N_VAL_SAMPLES)] + obstacle_heights[N_TRAIN_SAMPLES: (N_TRAIN_SAMPLES + N_VAL_SAMPLES)],\
    obstacle_start_xcoords[N_TRAIN_SAMPLES: (N_TRAIN_SAMPLES + N_VAL_SAMPLES)] + obstacle_widths[N_TRAIN_SAMPLES: (N_TRAIN_SAMPLES + N_VAL_SAMPLES)]
...
test_obs_ycoords_1, test_obs_xcoords_1 = \
    obstacle_start_ycoords[-N_TEST_SAMPLES:], obstacle_start_xcoords[-N_TEST_SAMPLES:]
test_obs_ycoords_2, test_obs_xcoords_2 = \
    obstacle_start_ycoords[-N_TEST_SAMPLES:] + obstacle_heights[-N_TEST_SAMPLES:],\
    obstacle_start_xcoords[-N_TEST_SAMPLES:] + obstacle_widths[-N_TEST_SAMPLES:]
...
obs_x1_pl = tf.placeholder(shape=(), dtype=tf.int32)
obs_x2_pl = tf.placeholder(shape=(), dtype=tf.int32)
obs_y1_pl = tf.placeholder(shape=(), dtype=tf.int32)
obs_y2_pl = tf.placeholder(shape=(), dtype=tf.int32)
...
velocity_0_div_free = divergence_free(velocity_0)
# velocities_tf = [velocity_0_div_free]
...
world.add(Obstacle(box[obs_y1_pl:obs_y2_pl, obs_x1_pl:obs_x2_pl]))

velocities_tf = [world.velocity]
...
for id in range(1):
    ...
    velocities = sess.run(velocities_tf, feed_dict={location_pl: location, strength_pl: strength, sigma_pl: sigma,
                                                    obs_y1_pl: train_obs_ycoords_1[id], obs_y2_pl: train_obs_ycoords_2[id],
                                                    obs_x1_pl: train_obs_xcoords_1[id], obs_x2_pl: train_obs_xcoords_2[id]})
