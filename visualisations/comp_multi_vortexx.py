...
parser.add_argument('--load_weights_ex', type=str, default='p2_r_T1_exp_weight_1.0_depth_2_100_batch_16_c2d_lr_1e-2_l2_1e-4_r256_1', help='name of the experiment to load weights from')
...
VortexNet = MultiParticleVortexNetwork(depth=opt.depth, hidden_units=opt.hidden_units, batch_norm=True,
...
losses= []
vortex_feature_list = [inp_feature]

for step in range(NUM_TIME_STEPS):

    vortex_feature = VortexNet(inp_feature.detach().clone())
    vortex_feature_list.append(vortex_feature)
    vel_y = falloff_kernel(vortex_feature, points_y)
    vel_yy, vel_yx = torch.unbind(vel_y, dim=-1)
    vel_x = falloff_kernel(vortex_feature, points_x)
    ...
    losses.append(F.mse_loss(vel, velocities_gpu[step + 1], reduction='sum').detach().clone())

loss_all = torch.stack(losses, dim=-1)
features = torch.stack(vortex_feature_list, dim=-1)